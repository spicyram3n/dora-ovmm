#!/usr/bin/env python3
"""Map one sphere of the workspace from the head RGBD stream.

    ros2 run nbv build_octomap.py --ros-args \
        -p sphere_centre:="[0.51, 0.0, 1.16]" -p sphere_radius:=0.5

Publishes the occupied cells on /nbv/occupied as a PointCloud2, to view in
RViz. Everything outside the sphere is dropped before it costs anything.
"""
...
from nbv.occupancy import SphereGrid
...
class OctomapBuilder(Node):
    def __init__(self):
        ...
        self.grid = SphereGrid(centre, radius, RESOLUTION)
        ...
        self.get_logger().info(
            f"sphere r={radius} at {centre} in {self.frame}, {self.grid.size}^3 cells")
    ...
    def on_depth(self, msg):
        ...
        self.grid.integrate(sensor, points @ rotation + sensor, empty @ rotation + sensor)

    def publish(self):
        cells = np.flatnonzero(self.grid.occupied())
        ...
        probability = 1 / (1 + np.exp(-self.grid.log_odds[cells]))
        points = np.column_stack([self.grid.centres(cells), probability])
        ...
        self.get_logger().info(
            f"occupied {len(cells)}  unknown {int(self.grid.unknown().sum())}",
    ...
    def sphere(self, header):
        ...
        (marker.pose.position.x, marker.pose.position.y,
         marker.pose.position.z) = self.grid.centre.tolist()
        marker.pose.orientation.w = 1.0
        diameter = 2 * self.grid.radius
