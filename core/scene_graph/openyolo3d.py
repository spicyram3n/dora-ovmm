...
def load_instances(manifest):
    ...
    instances = [Instance(item['label'], np.load(manifest.parent / item['points'], allow_pickle=False),
                          confidence=item['confidence'], name=item['name'])
...
def main():
    ...
    scene = sg.build(instances, source_frame=source_frame,
