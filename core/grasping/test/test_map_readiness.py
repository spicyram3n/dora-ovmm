...
class MapReadiness(unittest.TestCase):
    def test_waits_for_real_map_before_freezing(self):
        ...
        responses=iter([NS(),NS(success=True),NS(),NS(scene=empty),NS(scene=ready),NS(valid=True)])
        events=[]
        def call(*args):
        ...
        # The octomap is emptied before the scene diff, keeping its republish small.
        self.assertEqual(events[:2],['/clear_octomap','/apply_planning_scene'])
    ...
    def test_empty_map_timeout_does_not_freeze_as_ready(self):
        responses=iter([NS(),NS(success=True),NS(),NS(scene=PlanningScene())])
